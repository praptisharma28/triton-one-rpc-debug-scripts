...
| `test-slot-compare.js` | Compares slot values across commitment levels | `ENDPOINT=host TOKEN=xxx node test-slot-compare.js` |
| `test-ordering.js` | Checks ordering of account/slot updates from the stream | `ENDPOINT=host TOKEN=xxx node test-ordering.js` |
...
| `test-h2-repro-flash.js` | Reproduces h2 protocol error caused by missing keepalive config (runs ~2-3h) | `ENDPOINT=host TOKEN=xxx node test-h2-repro-flash.js` |
